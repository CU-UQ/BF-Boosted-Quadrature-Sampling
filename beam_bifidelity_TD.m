...
corr_lev_boosted = zeros(no_trials,2);
corr_lev_vol = zeros(no_trials,2);
corr_lev_vol_boosted = zeros(no_trials,2);
corr_unif = zeros(no_trials,2);
corr_unif_boosted = zeros(no_trials,2);
...
err_lev_vol_sampling = zeros(no_trials,1);
...
for tr = 1:no_trials
    [x_sol, ~] = total_degree_sampling(N,D,J,An,Un_sq,no_samp,y,'alev');
 ...
 % Boosted leverage score sampling
    best_err = Inf;
    for smp = 1:resamp
        [x_sol,~,samp_list, scaling] = total_degree_sampling(N,D,J,An,Un_sq,no_samp,yL,'alev');
    ...
    SA = A(best_samp, :);
    Sy = yH(best_samp);
    corr_lev_boosted(tr,1) = mu(A,best_samp,yL);
    corr_lev_boosted(tr,2) = mu(A,best_samp,yH);
    x_sol = SA \ Sy;
    err_lev_score_boosted(tr) = norm(A*x_sol - yH)/norm(yH);
 
 % Uniform sampling
    [x_sol, ~] = total_degree_sampling(N,D,J,An,Un_sq,no_samp,y,'unif');
 ...
 % Boosted leverage score sampling
    best_err = Inf;
    for smp = 1:resamp
        [x_sol,~,samp_list, scaling] = total_degree_sampling(N,D,J,An,Un_sq,no_samp,yL,'unif');
        current_err = norm(A*x_sol - yL)/norm(yL);
        err_all_boosted(tr, smp) = current_err;
        if current_err < best_err
            best_samp = samp_list;
    ...
    SA = A(best_samp, :);
    Sy = yH(best_samp);
    corr_unif_boosted(tr,1) = mu(A,best_samp,yL);
    corr_unif_boosted(tr,2) = mu(A,best_samp,yH);
    x_sol = SA \ Sy;
    ...
    SA = A(best_samp, :);
    Sy = yH(best_samp);
    corr_lev_vol_boosted(tr,1) = mu(A,best_samp,yL);
    corr_lev_vol_boosted(tr,2) = mu(A,best_samp,yH);
    x_sol = SA \ Sy;   
...
save('B_no_30_TD.mat','err_exact','err_QR','err_lev_score','err_lev_score_boosted','err_unif','err_unif_boosted','err_lev_vol','err_lev_vol_boosted');
save('B_Cor_no_30_TD.mat','corr_lev','corr_unif','corr_lev_vol');
